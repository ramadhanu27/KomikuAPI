import { load } from 'cheerio';

const scrapeCompleteManga = (html) => {
    const $ = load(html);
    const completeManga = [];
    
    $('.bgei').each((i, el) => {
        const $item = $(el);
        const $link = $item.find('a').first();
        const title = $item.find('h3, .kan').first().text().trim();
        const url = $link.attr('href');
        const poster = $item.find('img').attr('src') || $item.find('img').attr('data-src');
        const type = $item.find('.tpe1').text().trim();
        const chapterInfo = $item.find('.new1').text().trim();
        
        // Extract slug from URL
        let slug = '';
        if (url) {
            const match = url.match(/manga\/([^\/]+)\//);
            slug = match ? match[1] : '';
        }
        
        // Fix poster URL if relative
        let fixedPoster = poster;
        if (poster && !poster.startsWith('http')) {
            fixedPoster = `https://komiku.org${poster}`;
        }
        
        if (title && url) {
            completeManga.push({
                title,
                slug,
                poster: fixedPoster,
                type: type || 'Manga',
                chapter_info: chapterInfo,
                status: 'Completed',
                url
            });
        }
    });
    
    return completeManga;
};

export default scrapeCompleteManga;
