...
const manhua = async (page = 1) => {
    for (const base of BASEURLS) {
        const url = `${base}/type/manhua/page/${page}/`;
        try {
            console.log(`🇨🇳 Scraping manhua: ${url}`);
            const { data } = await client.get(url);
            console.log(`✅ Berhasil dari: ${url}`);
            const results = scrapeSearchResult(data);
            return results.filter(manga => manga.type && manga.type.toLowerCase().includes('manhua'));
        } catch (error) {
            console.error(`⚠️ Gagal: ${url} — ${error.code || error.message}`);
        }
    }
    throw new Error('❌ Tidak bisa mengakses manhua dari salah satu domain.');
};
